...
def test_canonical_rich_structure_matches_ground_truth(
    tmp_path: Path, provenance_pdf: Path, ground_truth: dict[str, object]
) -> None:
    result, _storage = _ingest_fixture(tmp_path, provenance_pdf)
    document = result.document

    assert document.title == ground_truth["document"]["title"]
    assert [page.printed_page_label for page in document.pages] == [
    ...
    assert {item.object_type for item in document.objects} >= {
        "table",
        "figure",
        "footnote",
    }
