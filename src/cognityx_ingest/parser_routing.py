...
class RoutingPlan:
    ...
    def to_extraction_policy(self) -> ExtractionPolicy:
        """Convert only a lossless accepted document plan to legacy policy.

        Compatibility-aware composition calls this before constructing an existing
        ``ParserRouter``. One purpose-free document invocation maps to ``fixed``;
        several map to existing ``compare``. Rejected plans, stop conditions,
        page scopes, security tags, or nonempty purposes raise
        ``ParserRoutingCompatibilityError`` rather than being discarded. The
        method is read-only and never runs or fuses parsers itself.
        """
        try:
...
class ParserRoutingService:
    ...
    Invariants:
        Deterministic mode never calls a provider; proposal modes call once; every
        proposal is untrusted; no method executes or fuses parser outputs.
    Lifecycle/persistence:
    ...
    def plan(
        ...
        *,
        ...
        LLM-directed mode, a provider. The method validates request/registry,
        evaluates typed rules or calls the provider exactly once, runs common
        deterministic validation, and returns an auditable plan. Provider errors
        become ``ParserRoutingProposalError``; capability and shape failures remain
...
def _validate_proposal(
    request: ParserRoutingRequest, proposal: RoutingProposal
...
def _validate_plan(plan: RoutingPlan) -> None:
    ...
    if plan.mode == "deterministic":
        if plan.input_facts is None or plan.llm_used or plan.proposal is not None:
            raise ParserRoutingValidationError(
